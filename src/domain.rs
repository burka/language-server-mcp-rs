...
impl RustAnalyzer {
    ...
    pub async fn hover(&self, file_path: &str, position: Position) -> Result<Option<HoverInfo>, LspError> {
        self.execute_with_retry("hover", || async {
            let client = self.lsp_client.lock().await;
            let result = client.hover(file_path, position.line, position.column).await?;
    ...
    pub async fn completion(&self, file_path: &str, position: Position) -> Result<Vec<CompletionItem>, LspError> {
        self.execute_with_retry("completion", || async {
            let client = self.lsp_client.lock().await;
            let result = client.completion(file_path, position.line, position.column).await?;
                ...
                Some(response) => {
                    ...
                    let completions = items
                        ...
                        .map(|item| CompletionItem {
                            label: item.label,
                            detail: item.detail,
                            kind: item.kind.map(|k| format!("{:?}", k)).unwrap_or_else(|| "Unknown".to_string()),
        ...
        self.execute_with_retry("diagnostics", || async {
            ...
            let diagnostics = result
                ...
                .map(|diag| Diagnostic {
                    ...
                    range: Range {
                        start: Position { line: diag.range.start.line, column: diag.range.start.character },
                        end: Position { line: diag.range.end.line, column: diag.range.end.character },
                    },
                    source: diag.source,
    ...
    pub async fn goto_definition(&self, file_path: &str, position: Position) -> Result<Vec<Location>, LspError> {
        self.execute_with_retry("goto_definition", || async {
            let client = self.lsp_client.lock().await;
            let result = client.goto_definition(file_path, position.line, position.column).await
                    ...
                    let locations = match response {
                        ...
                        GotoDefinitionResponse::Link(links) => {
                            return Ok(links.into_iter().map(|link| Location {
                                file_path: link.target_uri.to_file_path().unwrap_or_default().to_string_lossy().to_string(),
                                range: Range {
                                    start: Position { line: link.target_range.start.line, column: link.target_range.start.character },
                                    end: Position { line: link.target_range.end.line, column: link.target_range.end.character },
                                },
                            }).collect());
                        }
                    };
                    
                    let locations = locations.into_iter().map(|loc| Location {
                        file_path: loc.uri.to_file_path().unwrap_or_default().to_string_lossy().to_string(),
                        range: Range {
                            start: Position { line: loc.range.start.line, column: loc.range.start.character },
                            end: Position { line: loc.range.end.line, column: loc.range.end.character },
                        },
                    }).collect();
    ...
    pub async fn find_references(&self, file_path: &str, position: Position, include_declaration: bool) -> Result<Vec<Location>, LspError> {
        self.execute_with_retry("find_references", || async {
            let client = self.lsp_client.lock().await;
            let result = client.find_references(file_path, position.line, position.column, include_declaration).await?;
    ...
    pub async fn format_document(&self, file_path: &str) -> Result<Option<String>, LspError> {
        self.execute_with_retry("format_document", || async {
            let client = self.lsp_client.lock().await;
            let result = client.format_document(file_path).await?;
            ...
            match result {
                Some(edits) => {
                    if edits.is_empty() {
                        ...
                        Ok(Some(format!("Formatting would apply {} edits to the file", edits.len())))
                    }
                }
    ...
    pub async fn workspace_symbols(&self, query: &str) -> Result<Vec<Symbol>, LspError> {
        self.execute_with_retry("workspace_symbols", || async {
            ...
            let symbols = result.unwrap_or_default().into_iter().map(|symbol| {
                ...
                Symbol {
                    ...
                    kind,
                    location: Location {
                        file_path: symbol.location.uri.to_file_path().unwrap_or_default().to_string_lossy().to_string(),
                        range: Range {
                            start: Position { line: symbol.location.range.start.line, column: symbol.location.range.start.character },
                            end: Position { line: symbol.location.range.end.line, column: symbol.location.range.end.character },
                        },
                    },
    ...
    pub async fn document_symbols(&self, file_path: &str) -> Result<Vec<Symbol>, LspError> {
        self.execute_with_retry("document_symbols", || async {
            let client = self.lsp_client.lock().await;
            let result = client.document_symbols(file_path, 0, crate::lsp_client::MAX_SYMBOLS_COUNT).await?;
            
            let symbols = result.into_iter().map(|symbol| {
                let kind = match symbol.kind {
                    lsp_types::SymbolKind::FUNCTION => SymbolKind::Function,
                    lsp_types::SymbolKind::VARIABLE => SymbolKind::Variable,
                    lsp_types::SymbolKind::CLASS => SymbolKind::Class,
                ...
                Symbol {
                    ...
                    location: Location {
                        ...
                        range: Range {
                            start: Position { line: symbol.range.start.line, column: symbol.range.start.character },
                            end: Position { line: symbol.range.end.line, column: symbol.range.end.character },
                        },
                    },
                    container_name: None, // Document symbols don't typically have container names
                }
            }).collect();
    ...
    pub async fn close_document(&self, file_path: &str) -> Result<String, LspError> {
        self.execute_with_retry("close_document", || async {
            let client = self.lsp_client.lock().await;
            client.close_document(file_path).await?;
            Ok("Document closed successfully".to_string())
        }).await
    }
    ...
    pub async fn rename(&self, _file_path: &str, _position: Position, _new_name: &str) -> Result<String, LspError> {
        Ok("Rename operation not fully implemented in domain service yet".to_string())
    }
    
    pub async fn code_actions(&self, _file_path: &str, _position: Position) -> Result<String, LspError> {
        Ok("Code actions not fully implemented in domain service yet".to_string())
    }
    
    pub async fn inlay_hints(&self, _file_path: &str) -> Result<String, LspError> {
        Ok("Inlay hints not fully implemented in domain service yet".to_string())
    }
    
    pub async fn expand_macro(&self, _file_path: &str, _position: Position) -> Result<String, LspError> {
        Ok("Macro expansion not fully implemented in domain service yet".to_string())
    }
    
    pub async fn signature_help(&self, _file_path: &str, _position: Position) -> Result<String, LspError> {
        Ok("Signature help not fully implemented in domain service yet".to_string())
    }
    
    pub async fn document_highlight(&self, _file_path: &str, _position: Position) -> Result<String, LspError> {
        Ok("Document highlight not fully implemented in domain service yet".to_string())
    }
    
    pub async fn selection_range(&self, _file_path: &str, _positions: Vec<Position>) -> Result<String, LspError> {
    ...
    pub async fn implementations(&self, _file_path: &str, _position: Position) -> Result<String, LspError> {
        Ok("Implementations not fully implemented in domain service yet".to_string())
    }
    ...
    async fn execute_with_retry<F, Fut, T>(&self, operation_name: &str, handler: F) -> Result<T, LspError>
    where
        F: Fn() -> Fut,
        Fut: std::future::Future<Output = Result<T, LspError>>,
    {
        ...
        Err(LspError::Other { message: format!("Operation '{}' failed after {} retries", operation_name, MAX_RETRIES) })
    }
}