...
impl RustAnalyzer {
        ...
        self.execute_with_retry("document_symbols", || async {
            ...
            let symbols = match result {
                ...
                lsp_types::DocumentSymbolResponse::Flat(symbol_info_vec) => {
                    symbol_info_vec.into_iter().map(|symbol| {
                        let kind = match symbol.kind {
                            lsp_types::SymbolKind::FUNCTION => SymbolKind::Function,
                            lsp_types::SymbolKind::VARIABLE => SymbolKind::Variable,
                            lsp_types::SymbolKind::CLASS => SymbolKind::Class,
                ...
                lsp_types::DocumentSymbolResponse::Nested(doc_symbols) => {
                    ...
                    doc_symbols.into_iter().map(|symbol| {
                        let kind = match symbol.kind {
                            lsp_types::SymbolKind::FUNCTION => SymbolKind::Function,
                            lsp_types::SymbolKind::VARIABLE => SymbolKind::Variable,
                            lsp_types::SymbolKind::CLASS => SymbolKind::Class,
    ...
    pub async fn close_document(&self, file_path: &str) -> Result<(usize, usize), LspError> {
        ...
        client.close_document(file_path).await
            .map_err(|e| LspError::Other { message: format!("Failed to close document: {}", e) })?;
    ...
    pub async fn rename(&self, file_path: &str, position: Position, new_name: &str) -> Result<String, LspError> {
        self.execute_with_retry("rename", || async {
            let client = self.lsp_client.lock().await;
            let result = client.rename(file_path, position.line, position.column, new_name).await
                ...
                Some(workspace_edit) => {
                    if workspace_edit.changes.is_none() && workspace_edit.document_changes.is_none() {
                        ...
                        let change_count = workspace_edit.changes
                            ...
                            .unwrap_or(0);
                        Ok(format!("Rename available with {} file changes", change_count))
                    }
                }
    ...
    pub async fn code_actions(&self, file_path: &str, position: Position) -> Result<String, LspError> {
        self.execute_with_retry("code_actions", || async {
            let client = self.lsp_client.lock().await;
            let result = client.code_actions(file_path, position.line, position.column).await
                .map_err(|e| e.to_string())?;
            ...
            match result {
                Some(actions) => {
                    if actions.is_empty() {
                        ...
                        let action_types: Vec<String> = actions.iter().map(|action| {
                            match action {
                        ...
                        Ok(format!("Found {} code actions: {}", actions.len(), action_types.join(", ")))
                    }
                }
    ...
    pub async fn inlay_hints(&self, file_path: &str) -> Result<String, LspError> {
        self.execute_with_retry("inlay_hints", || async {
            let client = self.lsp_client.lock().await;
            let result = client.inlay_hints(file_path).await
                .map_err(|e| e.to_string())?;
    ...
    pub async fn expand_macro(&self, file_path: &str, position: Position) -> Result<String, LspError> {
        self.execute_with_retry("expand_macro", || async {
            let client = self.lsp_client.lock().await;
            let result = client.expand_macro(file_path, position.line, position.column).await
                .map_err(|e| e.to_string())?;
    ...
    pub async fn signature_help(&self, file_path: &str, position: Position) -> Result<String, LspError> {
        self.execute_with_retry("signature_help", || async {
            let client = self.lsp_client.lock().await;
            let result = client.signature_help(file_path, position.line, position.column).await
                .map_err(|e| e.to_string())?;
                                ...
                                .map(|params| {
                                    params.iter().map(|p| {
                                        match &p.label {
                                            ...
                                            lsp_types::ParameterLabel::LabelOffsets(_) => "param".to_string(),
                                        }
                                    }).collect::<Vec<_>>().join(", ")
                                })
                                .unwrap_or_default();
    ...
    pub async fn document_highlight(&self, file_path: &str, position: Position) -> Result<String, LspError> {
        self.execute_with_retry("document_highlight", || async {
            let client = self.lsp_client.lock().await;
            let result = client.document_highlight(file_path, position.line, position.column).await
                .map_err(|e| e.to_string())?;
    ...
    pub async fn implementations(&self, file_path: &str, position: Position) -> Result<String, LspError> {
        self.execute_with_retry("implementations", || async {
            let client = self.lsp_client.lock().await;
            let result = client.implementations(file_path, position.line, position.column).await
                .map_err(|e| e.to_string())?;
