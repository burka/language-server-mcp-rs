...
async fn test_fast_shared_multiple_requests() {
    ...
    for i in 0..3 {
        ...
        assert!(
            ...
            "Server should be available on access {}",
            i
        );
        drop(server_guard);
    }
    ...
    assert!(
        ...
        "Server access should be very fast"
    );
}
...
async fn test_benchmark_fresh_vs_shared() {
    ...
    let server_container = get_test_mcp_server().await;
    let server_guard = server_container.lock().await;
    let shared_server = server_guard
        .as_ref()
        .expect("Shared server should be initialized");
    let shared_result = shared_server
        .hover(Parameters(HoverRequest {
            file_path: "src/main.rs".to_string(),
            line: 18,
            column: 10,
    ...
    assert!(
        ...
        "Total test should complete reasonably quickly"
    );
}
