...
fn analyze_result(test_name: &str, code_snippet: &str, result: &CallToolResult) {
    ...
    if content_text.is_empty() {
        ...
        println!(
            "✅ Result: {} characters of semantic data",
            content_text.len()
        );
        println!("📄 Content:");
        // Show first 500 chars for document symbols, 200 for others
        let limit = if test_name.contains("Document Symbols") {
            500
        } else {
            200
        };
    ...
    match server.hover(Parameters(hover_request)).await {
        Ok(result) => {
            ...
            assert!(
                ...
                "Function hover should return semantic info"
            );
            ...
            if content_text.contains("fn main") || content_text.contains("async") {
                ...
                println!(
                    "⚠️  May not contain expected function signature - got: {}",
                    content_text
                );
            }
        }
    ...
    match server.hover(Parameters(struct_hover)).await {
        Ok(result) => {
            ...
            if content_text.contains("trait") || content_text.contains("Parser") {
                ...
                println!(
                    "⚠️  Got semantic info but may not be trait-specific: {}",
                    content_text
                );
            }
        }
    ...
    match server.hover(Parameters(var_hover)).await {
        Ok(result) => {
            ...
            if content_text.contains("Args") || content_text.contains("type") {
                ...
                println!(
                    "⚠️  Got info but may not be type-specific: {}",
                    content_text
                );
            }
        }
    ...
    match server.hover(Parameters(empty_hover)).await {
        Ok(result) => {
            ...
            if content_text.is_empty() {
                ...
                println!(
                    "⚠️  Unexpectedly got content for whitespace: {}",
                    content_text
                );
            }
        }
    ...
    match server.completion(Parameters(method_completion)).await {
        Ok(result) => {
            ...
            if content_text.contains("parse") || content_text.contains("method") {
                ...
                println!(
                    "⚠️  Got completions but may not contain 'parse': {}",
    ...
    match server.goto_definition(Parameters(goto_request)).await {
        Ok(result) => {
            ...
            if content_text.contains("struct") || content_text.contains("Args") {
                ...
                println!(
                    "⚠️  Got definition but may not be struct-specific: {}",
                    content_text
                );
            }
        }
...
async fn test_validate_expected_semantic_responses() {
    ...
    match server.document_symbols(Parameters(symbols_request)).await {
        Ok(result) => {
            ...
            println!(
                "✅ Document symbols response ({} chars):",
                content_text.len()
            );
        ...
        match server.hover(Parameters(request.clone())).await {
            Ok(result) => {
                ...
                if !content_text.contains("No hover information") && content_text.len() > 50 {
                    println!(
                        "✅ First attempt success for '{}': {} chars",
                        code_snippet,
                        content_text.len()
                    );
                ...
                println!(
                    "⚠️  First attempt gave poor result for '{}': '{}'",
                    code_snippet, content_text
                );

                // Check if this might be an indexing issue
                if content_text.contains("No hover information")
                    || content_text.contains("not available")
                {
                    ...
                    match server.hover(Parameters(request)).await {
                        Ok(retry_result) => {
                            ...
                            if !retry_content.contains("No hover information")
                                && retry_content.len() > 50
                            {
                                println!(
                                    "✅ Retry success for '{}' after indexing wait: {} chars",
                                    code_snippet,
                                    retry_content.len()
                                );
        ...
        match smart_hover_with_indexing_check(&server, file_path, line, column, description).await {
            Ok(content) => {
                if content.contains("No hover information") || content.len() < 30 {
                    println!(
                        "⚠️  {}: Limited semantic info - may need more indexing time",
                        description
                    );
                    println!("   Content: '{}'", content);
                } else {
                    println!(
                        "✅ {}: Good semantic response ({} chars)",
                        description,
                        content.len()
                    );
                    println!(
