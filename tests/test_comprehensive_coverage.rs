...
async fn get_shared_server() -> Arc<Mutex<RustAnalyzerMCP>> {
    SHARED_SERVER.get_or_init(|| async {
...
async fn test_comprehensive_selection_range() {
    ...
    let request = language_server_mcp::models::SelectionRangeRequest {
        file_path: "src/main.rs".to_string(),
        positions: vec![
            language_server_mcp::models::PositionInfo { line: 13, column: 7 },
            language_server_mcp::models::PositionInfo { line: 20, column: 10 },
        ],
    };
...
async fn test_hanging_scenario_deep_goto_definition() {
    ...
    match result {
        Ok(tool_result) => {
            println!("Deep goto definition succeeded: has_content={}", has_content(&tool_result));
            assert!(has_content(&tool_result));
        }
        Err(e) => println!("Deep goto definition error (may indicate hang issue): {:?}", e),
    }
}
...
async fn test_hanging_scenario_complex_completion() {
    ...
    match result {
        Ok(tool_result) => {
            println!("Complex completion succeeded: has_content={}", has_content(&tool_result));
            assert!(has_content(&tool_result));
        }
        Err(e) => println!("Complex completion error (may indicate hang issue): {:?}", e),
    }
}
...
async fn test_hanging_scenario_workspace_wide_search() {
    ...
    match result {
        Ok(tool_result) => {
            println!("Workspace search succeeded: has_content={}", has_content(&tool_result));
            assert!(has_content(&tool_result));
        }
