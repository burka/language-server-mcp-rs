...
async fn test_throttle_delay_sweep() {
    ...
    for delay_ms in test_delays {
        ...
        println!("    Success: {}/{} ({:.1}%)", successful, results.len(), success_rate);
        println!("    Failed: {}", failed);
        println!("    Total time: {:?}", total_time);
        println!("    Avg time per request: {:?}", total_time / results.len() as u32);
        ...
        if success_rate == 100.0 {
            ...
            println!("    ✅ GOOD: {}ms provides excellent success rate", delay_ms);
...
async fn test_throttle_vs_no_throttle() {
    ...
    println!("  No throttle: {}/{} success in {:?}", 
        success_no_throttle, results_no_throttle.len(), time_no_throttle);
    ...
    println!("  With 75ms throttle: {}/{} success in {:?}", 
        success_throttled, results_throttled.len(), time_throttled);
    
    // Analysis
    println!("\n📊 Comparison Summary:");
    println!("  Success rate improvement: {} → {} requests", 
        success_no_throttle, success_throttled);
    println!("  Time trade-off: {:?} → {:?}", time_no_throttle, time_throttled);
    
    let improvement = success_throttled as i32 - success_no_throttle as i32;
    if improvement > 0 {
        println!("  🎯 Throttling improved success by {} requests!", improvement);
...
async fn test_optimal_throttle_recommendation() {
    ...
    for (delay_ms, description) in candidates {
        ...
        for i in 0..4 {
            ...
            let diag_future = server.diagnostics(Parameters(DiagnosticsRequest {
                file_path: format!("src/{}.rs", match i {
                    0 => "server",
                    1 => "lsp_client", 
                    2 => "models",
                    _ => "tool_handlers",
        ...
        println!("  {}ms ({}): {:.1}% success ({}/{})", 
            delay_ms, description, success_rate, successful, results.len());
    ...
    if best_delay != 25 && best_success_rate > 90.0 {
        println!("  💡 Consider updating default from 25ms to {}ms", best_delay);
    }
