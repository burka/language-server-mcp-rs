...
async fn test_ultimate_lsp_throttling() {
    ...
    println!(
        "✅ Ultimate LSP throttling enabled: {}ms delay",
    ...
    println!(
        "🚀 Starting {} requests with ultimate LSP throttling...",
        test_requests.len()
    );
    ...
    assert!(
        successful >= 3,
        "Most requests should succeed with LSP throttling"
    );

    // Should take time due to throttling (3 delays * 50ms ≈ 150ms minimum)
    assert!(
        total_time >= Duration::from_millis(100),
        "Should take at least ~150ms with 50ms throttling: {:?}",
        total_time
    );
...
async fn test_throttling_at_protocol_level() {
    ...
    println!(
        "🔧 Throttling configured at LSP level: {}ms",
        server.get_throttle_delay_ms()
    );
    ...
    println!(
        "  Hover result: {}",
        if hover_result.is_ok() { "✅" } else { "❌" }
    );
    println!(
        "  Diagnostics result: {}",
        if diag_result.is_ok() { "✅" } else { "❌" }
    );
    println!("  Total time: {:?}", total_time);
