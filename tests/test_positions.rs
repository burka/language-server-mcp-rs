...
pub mod main_rs {
    ...
    pub const MAIN_FUNCTION: (u32, u32) = (179, 8); // "fn main()"
    pub const HOVER_TOOL: (u32, u32) = (65, 8);     // "async fn hover_tool"
    
    // Type names (good for hover, find_references)  
    pub const HOVER_REQUEST: (u32, u32) = (68, 37);  // "HoverRequest" parameter
}
...
pub mod lsp_client_rs {
    ...
    // Struct definitions
    pub const LSP_CLIENT_STRUCT: (u32, u32) = (32, 11); // "LspClient" in struct definition
    
    // Method definitions  
    pub const NEW_METHOD: (u32, u32) = (92, 18);         // "pub async fn new"
    pub const HOVER_METHOD: (u32, u32) = (357, 18);      // "pub async fn hover"
}
...
pub mod errors_rs {
    ...
    // Enum definition
    pub const LSP_ERROR_ENUM: (u32, u32) = (4, 11);           // "LspError"
    
    // Enum variants (good for find_references)
    pub const RUST_ANALYZER_NOT_FOUND: (u32, u32) = (5, 4);   // "RustAnalyzerNotFound"
    pub const TIMEOUT_ERROR: (u32, u32) = (9, 4);             // "TimeoutError"
}
        ...
        for (line, column) in positions {
            ...
            if line_idx >= lines.len() {
                return Err(format!(
                    ...
                    line, column, file_path, lines.len()
                ));
            }
            ...
            if col_idx >= lines[line_idx].len() {
                return Err(format!(
                    ...
                    line, column, file_path, line, lines[line_idx].len()
                ));
            }
        }
    }
...
mod tests {
    ...
    async fn test_validate_positions() {
        validate_positions().await.expect("Test positions should be valid");
    }
}