...
async fn test_diagnostics_success() {
    ...
    match result {
        Ok(Ok(diagnostics)) => {
            println!(
                "✅ Diagnostics succeeded with {} issues found",
                diagnostics.len()
            );
...
async fn test_diagnostics_multiple_files() {
    ...
    for file in test_files {
        ...
        match result {
            Ok(Ok(diagnostics)) => {
                println!(
                    "✅ Diagnostics for {} succeeded: {} issues",
                    file,
                    diagnostics.len()
                );
            }
...
async fn test_diagnostics_invalid_files() {
    ...
    for (file, description) in invalid_files {
        ...
        match result {
            Ok(Ok(diagnostics)) => {
                println!(
                    "⚠️  {} returned {} diagnostics (unexpected success)",
                    description,
                    diagnostics.len()
                );
            }
...
async fn test_diagnostics_hang_detection() {
    ...
    for i in 0..5 {
        ...
        match result {
            Ok(Ok(diagnostics)) => {
                println!(
                    "✅ Rapid diagnostics {} succeeded: {} issues",
                    i,
                    diagnostics.len()
                );
            }
...
async fn test_diagnostics_concurrent() {
    ...
    let tasks: Vec<_> = files
        ...
        .map(|(i, file)| async move {
            ...
            match result {
                Ok(Ok(diagnostics)) => {
                    println!(
                        "✅ Concurrent diagnostics {} succeeded: {} issues",
                        i,
                        diagnostics.len()
                    );
    ...
    println!(
        "📊 Concurrent diagnostics: {}/{} successful, {} total issues found",
        successful,
        files.len(),
        total_issues
    );
