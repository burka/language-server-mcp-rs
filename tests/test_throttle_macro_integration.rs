...
async fn test_macro_throttling_integration() {
    ...
    println!("✅ Server created with throttling: {}ms delay", server.get_throttle_delay_ms());
    ...
    assert!(successful >= 4, "Most requests should succeed with macro throttling");
    
    // Should take at least some time due to throttling (4 delays * 25ms ≈ 100ms minimum)
    assert!(total_time >= Duration::from_millis(80), 
        "Should take at least ~100ms with 25ms throttling: {:?}", total_time);
    
    // But not too long (should be much faster than serial execution)
    assert!(total_time < Duration::from_secs(8), 
        "Should complete in reasonable time with throttling: {:?}", total_time);
...
async fn test_throttled_vs_non_throttled() {
    ...
    println!("  Non-throttled: {}/{} success in {:?}", 
        successful_normal, results_normal.len(), time_normal);
    ...
    println!("  Throttled: {}/{} success in {:?}", 
        successful_throttled, results_throttled.len(), time_throttled);
    
    // Throttling should improve success rate
    assert!(successful_throttled >= successful_normal, 
        "Throttling should improve or maintain success rate");
    
    println!("📈 Improvement: {} → {} successful requests", 
