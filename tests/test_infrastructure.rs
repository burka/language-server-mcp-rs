...
async fn test_lsp_client_creation() {
    ...
    match result {
        Ok(client) => {
            ...
            println!(
                "📊 Opened documents count: {}",
                client.get_opened_documents_count().await
            );
        }
        Err(e) => {
            if format!("{}", e).contains("rust-analyzer not found") {
                println!(
                    "⚠️  rust-analyzer not installed - this is expected in some CI environments"
                );
