...
async fn test_rust_analyzer_sequential_baseline() {
    ...
    for (i, (method, file, line, column)) in requests.into_iter().enumerate() {
        ...
        match result {
            Ok(_) => println!("✅ Request {}: {} completed in {:?}", i+1, method, duration),
            Err(e) => println!("⚠️  Request {}: {} error in {:?}: {}", i+1, method, duration, e),
        }
    }
    ...
    println!("  Average per request: {:?}", total_time / individual_times.len() as u32);
}
...
async fn test_concurrent_same_file_requests() {
    ...
    println!("📄 Testing {} concurrent requests on same file...", futures.len());
    let start = Instant::now();
