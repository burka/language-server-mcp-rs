...
async fn test_option_a_multiple_servers() {
    ...
    let avg_creation_time: std::time::Duration = creation_times.iter().sum::<std::time::Duration>() / creation_times.len() as u32;
...
async fn test_option_a_error_handling() {
    ...
    println!("🔍 Nonexistent file hover result in {:?}: {:?}", duration, 
