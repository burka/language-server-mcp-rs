...
async fn test_staggered_requests_20ms() {
    ...
    let warmup_result = server.hover(Parameters(HoverRequest {
...
async fn test_staggered_requests_50ms() {
    ...
    // Warmup
    println!("🔄 Warming up...");
    let _ = server.hover(Parameters(HoverRequest {
        file_path: "src/main.rs".to_string(),
        line: 18,
        column: 15,
    })).await;
...
async fn test_optimal_stagger_timing() {
    ...
    for delay_ms in stagger_delays {
        ...
        println!("   {}ms delay: {}/{} success, {:?} total time", 
                delay_ms, successful, results.len(), total_time);
...
async fn test_mixed_operations_staggered() {
    ...
    // Warmup
    let _ = server.diagnostics(Parameters(DiagnosticsRequest {
        file_path: "src/main.rs".to_string(),
    })).await;
    ...
    let (_hover_result, _diag_result, _goto_result) = 
        futures::future::try_join3(hover_future, diag_future, goto_future).await
        .unwrap_or_else(|_| {
            // If any fail, handle individually
            futures::executor::block_on(async {
                let h = server.hover(Parameters(HoverRequest {
                    file_path: "src/server.rs".to_string(),
                    line: 50, column: 10,
                })).await;
                let d = server.diagnostics(Parameters(DiagnosticsRequest {
                    file_path: "src/lsp_client.rs".to_string(),
                })).await;
                let g = server.goto_definition(Parameters(GotoDefinitionRequest {
                    file_path: "src/models.rs".to_string(),
                    line: 15, column: 10,
                })).await;
...
async fn test_production_like_staggered_scenario() {
    ...
    let warmup = server.hover(Parameters(HoverRequest {
        file_path: "src/main.rs".to_string(),
        line: 18, column: 15,
    })).await;
    ...
    assert!(successful >= 6, "Most tests should succeed with proper staggering");
}